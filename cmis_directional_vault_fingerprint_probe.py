#!/usr/bin/env python3
"""Read-only CMIS v1.4.3 direction-aware vault-fingerprint probe."""
...
def main():
    ...
    observed = any(
        (item.get("summary") or {}).get(
            "directional_vault_pair_correlation_observed"
        ) is True
        for item in reports
    )

    result = {
        "service": "directional_vault_fingerprint_probe",
        "version": "1.4.3",
        "chain": "x1",
        "asset": dict(resolved),
        "status": (
            "stable_directional_pair_correlation_observed"
            if all_ranges_proven and observed
            else "observed_no_stable_directional_pair"
        ...
        "summary": {
            ...
            "directional_vault_pair_correlation_observed": observed,
            "canonical_vault_mapping_proven": False,
            "canonical_vault_mapping_promoted": False,
            "exact_pool_leg_semantics_promoted": False,
            "interpretation": (
                "v1.4.3 measures instruction-position stability separately "
                "for BUY-like and SELL-like flows. Promotion remains disabled."
            ),
        },
        "errors": [],
    }
