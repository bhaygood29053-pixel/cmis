...
class X1ReserveScopeArtifactTests(unittest.TestCase):
    def test_builds_sanitized_replayable_artifact_without_promotion(self):
        ...
        self.assertEqual(result["roles"]["asset"]["provider_raw_value"], "42.5")
        self.assertEqual(
            result["roles"]["asset"]["rpc_balance"]["amount"], "42500000"
        )
    ...
    def test_upstream_promotion_claims_never_promote_artifact(self):
        ...
        self.assertTrue(result["verification_state"]["upstream_bundle_cmis_promotable"])
        self.assertTrue(result["verification_state"]["upstream_scope_cmis_promotable"])
        self.assertTrue(result["verification_state"]["freshness_verified"])
        self.assertFalse(result["cmis_promotable"])

    def test_identity_rejection_reasons_are_preserved(self):
        input_bundle = bundle()
        verification = input_bundle["roles"]["asset"]["rpc_identity_verification"]
    ...
    def test_inconsistent_expected_authority_is_recorded_not_hidden(self):
        input_bundle = bundle()
        input_bundle["roles"]["counter"]["expected"]["shared_authority"] = "other-owner"
        ...
        self.assertIn(
            "expected_shared_authority_inconsistent_or_missing", result["warnings"]
        )
        self.assertFalse(result["cmis_promotable"])
