...
def dust_snapshot(**overrides):
    snapshot = {
        ...
        # The collector snapshot keeps the separately proven continuous CP
        # reconstruction. The quote-side XDEX price-impact semantic is based on
        # integer-rounded token output and therefore differs materially here.
...
class XDEXIntegerRoundedPriceImpactTests(unittest.TestCase):
    ...
    def test_continuous_snapshot_reconstruction_is_still_independently_verified(self):
        with self.assertRaisesRegex(
            XDEXRouteResolverError,
            ...
            resolve_xdex_route_evidence(
                ...
                "1000",
                collector=lambda route, amount: dust_snapshot(
                    reconstructed_price_impact_percent="98.2143",
                ),
            )

    def test_zero_integer_output_fails_closed_instead_of_reporting_zero_impact(self):
        with self.assertRaisesRegex(
            XDEXRouteResolverError,
            "integer-rounded CP output is zero",
        ):
            resolve_xdex_route_evidence(
                ROUTE,
                "0.000002",
