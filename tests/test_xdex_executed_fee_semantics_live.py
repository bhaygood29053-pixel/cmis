...
def _diagnostic_fee_logs(tx):
    ...
    for line in meta.get("logMessages") or []:
        ...
        if not match:
            continue
        row = {key: int(value) if key != "creator_on_input" else value == "true" for key, value in match.groupdict().items()}
        rows.append(row)
    return rows
...
class XDEXExecutedFeeSemanticLiveTests(unittest.TestCase):
    def test_completed_swaps_localize_2800_vs_3000_execution_semantics(self):
        ...
        # If direct fee evidence exists, it must be internally decisive rather than
        # silently accepting contradictory 2800/3000 classifications.
        for row in direct_fee_rows:
            self.assertFalse(
