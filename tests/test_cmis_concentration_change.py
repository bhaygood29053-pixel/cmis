from datetime import datetime, timezone
import unittest

from liquidity_scout.cmis.concentration import build_top_account_concentration
from liquidity_scout.cmis.concentration_change import compare_top_account_concentration


class ConcentrationChangeTests(unittest.TestCase):
    def observation(
        self,
        amounts,
        *,
        supply=1000,
        source="x1_rpc",
        requested_limit=20,
        supply_identity_verified=True,
        account_identity_verified=True,
    ):
        return build_top_account_concentration(
            chain="x1",
            asset_id="mint-1",
            source=source,
            supply_raw=supply,
            supply_decimals=0,
            requested_account_limit=requested_limit,
            accounts=[
                {"address": f"acct-{i}", "amount": amount, "decimals": 0}
                for i, amount in enumerate(amounts)
            ],
            supply_identity_verified=supply_identity_verified,
            account_identity_verified=account_identity_verified,
        )

    def compare(self, before, after):
        return compare_top_account_concentration(
            before=before,
            after=after,
            before_observed_at=datetime(2026, 8, 18, 10, tzinfo=timezone.utc),
            after_observed_at=datetime(2026, 8, 18, 11, tzinfo=timezone.utc),
        )

    def test_reports_numeric_increase_without_behavior_label(self):
        result = self.compare(self.observation([200, 100]), self.observation([250, 100]))
        self.assertEqual(result["direction"], "INCREASE")
        self.assertEqual(
            result["delta_share_exact"],
            {"numerator": "1", "denominator": "20"},
        )
        self.assertEqual(result["delta_share"], "0.05")
        self.assertEqual(result["delta_bps"], "500")
        self.assertTrue(result["identity_verified"])
        self.assertFalse(result["behavioral_interpretation_verified"])
        self.assertFalse(result["cmis_promotable"])
        self.assertTrue(result["before_observed_at"].endswith("Z"))
        self.assertTrue(result["after_observed_at"].endswith("Z"))

    def test_reports_decrease_and_no_change(self):
        self.assertEqual(
            self.compare(self.observation([300]), self.observation([200]))["direction"],
            "DECREASE",
        )
        self.assertEqual(
            self.compare(self.observation([200]), self.observation([200]))["direction"],
            "NO_CHANGE",
        )

    def test_nonterminating_decimal_change_uses_exact_raw_fraction(self):
        result = self.compare(
            self.observation([1], supply=3, requested_limit=1),
            self.observation([2], supply=3, requested_limit=1),
        )
        self.assertEqual(
            result["before_share_exact"],
            {"numerator": "1", "denominator": "3"},
        )
        self.assertEqual(
            result["after_share_exact"],
            {"numerator": "2", "denominator": "3"},
        )
        self.assertEqual(
            result["delta_share_exact"],
            {"numerator": "1", "denominator": "3"},
        )
        self.assertTrue(result["delta_share"].startswith("0.333333333333"))

    def test_rejects_incompatible_asset_or_source(self):
        after = self.observation([200])
        after["asset_id"] = "other-mint"
        with self.assertRaisesRegex(ValueError, "matching asset_id"):
            self.compare(self.observation([200]), after)

        with self.assertRaisesRegex(ValueError, "matching source"):
            self.compare(
                self.observation([200], source="x1_rpc"),
                self.observation([200], source="other_rpc"),
            )

    def test_rejects_different_top_n_scope_or_observed_count(self):
        with self.assertRaisesRegex(ValueError, "matching requested_account_limit"):
            self.compare(
                self.observation([200], requested_limit=20),
                self.observation([200], requested_limit=10),
            )

        with self.assertRaisesRegex(ValueError, "matching observed_account_count"):
            self.compare(
                self.observation([200], requested_limit=20),
                self.observation([200, 0], requested_limit=20),
            )

    def test_rejects_unverified_identity(self):
        unverified = self.observation([200], account_identity_verified=False)
        with self.assertRaisesRegex(ValueError, "verified asset/account identity"):
            self.compare(unverified, self.observation([200]))

    def test_rejects_forged_scope_or_promotion_flags(self):
        after = self.observation([200])
        after["scope_complete"] = True
        with self.assertRaises(ValueError):
            self.compare(self.observation([200]), after)

        after = self.observation([200])
        after["cmis_promotable"] = True
        with self.assertRaises(ValueError):
            self.compare(self.observation([200]), after)

        after = self.observation([200])
        after["scope_limit_filled"] = True
        with self.assertRaisesRegex(ValueError, "scope_limit_filled"):
            self.compare(self.observation([200]), after)

    def test_rejects_forged_exact_ratio(self):
        after = self.observation([200])
        after["observed_share_exact"] = {"numerator": "201", "denominator": "1000"}
        with self.assertRaisesRegex(ValueError, "must match observed_balance_raw/supply_raw"):
            self.compare(self.observation([200]), after)

    def test_rejects_zero_supply_observation_without_share(self):
        zero = build_top_account_concentration(
            chain="x1",
            asset_id="mint-1",
            source="x1_rpc",
            supply_raw=0,
            supply_decimals=0,
            requested_account_limit=20,
            accounts=[],
            supply_identity_verified=True,
            account_identity_verified=True,
        )
        with self.assertRaises(ValueError):
            self.compare(zero, self.observation([0]))

    def test_requires_ordered_timezone_aware_times(self):
        before = self.observation([200])
        after = self.observation([200])
        with self.assertRaises(ValueError):
            compare_top_account_concentration(
                before=before,
                after=after,
                before_observed_at=datetime(2026, 8, 18, 10),
                after_observed_at=datetime(2026, 8, 18, 11, tzinfo=timezone.utc),
            )
        with self.assertRaises(ValueError):
            compare_top_account_concentration(
                before=before,
                after=after,
                before_observed_at=datetime(2026, 8, 18, 11, tzinfo=timezone.utc),
                after_observed_at=datetime(2026, 8, 18, 10, tzinfo=timezone.utc),
            )


if __name__ == "__main__":
    unittest.main()
