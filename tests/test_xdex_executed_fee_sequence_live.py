...
def _longest_state_contiguous_chain(rows):
    by_pre = {row["pre_state"]: row for row in rows}
    post_states = {row["post_state"] for row in rows}
    starts = [row for row in rows if row["pre_state"] not in post_states] or list(rows)
    best = []
    for start in starts:
        chain = []
        seen = set()
...
class XDEXExecutedFeeSequenceLiveTests(unittest.TestCase):
    def test_contiguous_completed_swap_sequence_strongly_favors_2800(self):
        ...
        chain = _opposite_direction_slice(_longest_state_contiguous_chain(rows))
