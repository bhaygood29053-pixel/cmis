...
def backing(*, amount=24_007_049, current=True, authority=True, equal=True):
    destination_amount = amount if equal else amount + 1
    return {
...
class X1UsdcxDestinationParityTests(unittest.TestCase):
    def test_current_exact_backing_closure_verifies_destination_parity(self):
        ...
        self.assertTrue(result["exact_route_identity_verified"])
        self.assertTrue(result["source_destination_identity_verified"])
    ...
    def test_supply_mismatch_fails_closed(self):
        result = evaluate_usdcx_destination_parity(backing(equal=False))
