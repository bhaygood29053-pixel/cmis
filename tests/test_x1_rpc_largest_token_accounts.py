...
def success(values=None):
    if values is None:
        values = [
            {"address": "acct1", "amount": "5000", "decimals": 6, "uiAmount": 0.005, "uiAmountString": "0.005"},
            {"address": "acct2", "amount": "4000", "decimals": 6, "uiAmount": 0.004, "uiAmountString": "0.004"},
        ]
    return {"jsonrpc": "2.0", "id": 1, "result": {"context": {"slot": 77}, "value": values}}
...
class X1RPCLargestTokenAccountsTests(unittest.TestCase):
    def test_preserves_raw_top_accounts_without_holder_promotion(self):
        ...
        self.assertEqual(session.calls[0][1]["params"], ["mint1", {"commitment": "confirmed"}])
    ...
    def test_rpc_error_fails_closed(self):
        with self.assertRaisesRegex(X1RPCLargestTokenAccountsError, "returned an error"):
            fetch_largest_token_accounts_raw(
                "mint1", rpc_url="https://rpc.example", session=Session({"error": {"code": -1}})
            )

    def test_invalid_slot_fails_closed(self):
        body = success()
        body["result"]["context"]["slot"] = True
        with self.assertRaisesRegex(X1RPCLargestTokenAccountsError, "slot is invalid"):
            fetch_largest_token_accounts_raw("mint1", rpc_url="https://rpc.example", session=Session(body))

    def test_invalid_amount_fails_closed(self):
        values = [{"address": "acct1", "amount": "1.5", "decimals": 6}]
        with self.assertRaisesRegex(X1RPCLargestTokenAccountsError, "amount is invalid"):
            fetch_largest_token_accounts_raw("mint1", rpc_url="https://rpc.example", session=Session(success(values)))
    ...
    def test_duplicate_accounts_fail_closed(self):
        ...
        with self.assertRaisesRegex(X1RPCLargestTokenAccountsError, "duplicate accounts"):
            fetch_largest_token_accounts_raw("mint1", rpc_url="https://rpc.example", session=Session(success(values)))
    ...
    def test_inconsistent_decimals_fail_closed(self):
        ...
        with self.assertRaisesRegex(X1RPCLargestTokenAccountsError, "decimals are inconsistent"):
            fetch_largest_token_accounts_raw("mint1", rpc_url="https://rpc.example", session=Session(success(values)))

    def test_empty_mint_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "mint must not be empty"):
            fetch_largest_token_accounts_raw("", rpc_url="https://rpc.example", session=Session(success()))
