...
class Issue99TradeSizePolicyTests(unittest.TestCase):
    def test_default_policy_is_named_versioned_and_explicit(self):
        self.assertEqual(VERSION, "2.0")
        self.assertEqual(
            DEFAULT_PRE_TRADE_POLICY["policy_name"],
            "cmis_x1_trade_size_conservative",
        )
        self.assertEqual(DEFAULT_PRE_TRADE_POLICY["policy_version"], "1.0")
        self.assertEqual(
            DEFAULT_PRE_TRADE_POLICY["low_max_notional_to_liquidity_ratio"],
            0.02,
        )
        self.assertEqual(
            DEFAULT_PRE_TRADE_POLICY["moderate_max_notional_to_liquidity_ratio"],
            0.05,
        )
        self.assertEqual(
            DEFAULT_PRE_TRADE_POLICY["high_max_notional_to_liquidity_ratio"],
            0.10,
        )

    def test_small_50_dollar_trade_is_low_at_verified_3380_liquidity(self):
        result = assess_trade_size_liquidity(_risk(), _trade(50))
    ...
    def test_medium_trade_is_moderate_then_high_as_ratio_crosses_policy(self):
        moderate = assess_trade_size_liquidity(_risk(), _trade(150))
        high = assess_trade_size_liquidity(_risk(), _trade(250))
        self.assertEqual(moderate["status"], "PASS")
        self.assertEqual(
            moderate["evidence"]["trade_size_classification"], "MODERATE"
        )
    ...
    def test_500_dollar_agi_example_is_very_high_and_blocked(self):
        result = assess_trade_size_liquidity(_risk(), _trade(500))
        evidence = result["evidence"]
    ...
    def test_market_large_2000_trade_is_very_high(self):
        result = assess_trade_size_liquidity(_risk(), _trade(2000))
        self.assertEqual(result["status"], "BLOCK")
        self.assertEqual(
            result["evidence"]["trade_size_classification"], "VERY_HIGH"
        )

    def test_sell_side_uses_same_verified_market_size_policy(self):
        result = assess_trade_size_liquidity(_risk(), _trade(1000, side="sell"))
        self.assertEqual(result["status"], "BLOCK")
        self.assertEqual(
            result["evidence"]["trade_size_classification"], "VERY_HIGH"
        )
    ...
    def test_missing_liquidity_fails_closed_without_fake_ratio(self):
        result = assess_trade_size_liquidity(
            _risk(liquidity=None, liquidity_verified=False), _trade(500)
        )
        self.assertEqual(result["status"], "BLOCK")
    ...
    def test_conflicting_or_unverified_liquidity_fails_closed(self):
        # A numeric provider value is not promotable when the upstream CMIS
        # confidence gate says liquidity is not verified (for example after a
        # cross-source disagreement).
        result = assess_trade_size_liquidity(
            _risk(liquidity=3380.0, liquidity_verified=False), _trade(500)
        )
        self.assertEqual(result["status"], "BLOCK")
        self.assertIsNone(result["evidence"]["notional_to_liquidity_ratio"])
    ...
    def test_execution_estimates_remain_explicitly_unavailable_without_proof(self):
        result = build_pre_trade_check(_risk(), _trade(50), chain="x1")
