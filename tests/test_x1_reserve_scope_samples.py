    ...
    return {
        ...
        "scope": {
            ...
            "metrics": {
                ...
                "roles": {
                    "asset": {
                        "balance_identity_slot_delta": asset_delta,
                    },
                    "counter": {
                        "balance_identity_slot_delta": counter_delta,
                    },
                },
            },
            "warnings": [],
...
class X1ReserveScopeSampleSummaryTests(unittest.TestCase):
    ...
    def test_even_sample_median_is_exact_decimal(self):
        ...
        self.assertEqual(
            result["metrics"]["collection_duration_seconds"]["median"],
            "3.5",
        )
    ...
    def test_missing_metric_is_partial_with_explicit_coverage(self):
        ...
        self.assertIn(
            "partial_metric_coverage:provider_sync_age_seconds",
            result["warnings"],
        )
        self.assertFalse(result["freshness_verified"])
    ...
    def test_negative_provider_sync_age_is_measured_and_flagged(self):
        result = summarize_x1_reserve_scope_artifacts(
            [artifact(provider_age="-2")]
        )

        self.assertEqual(
            result["metrics"]["provider_sync_age_seconds"]["min"],
            "-2",
        )
