...
- **Issue #313 — timestamped secondary Solana price evidence: implemented in PR #314.** CMIS reads one exact USDC/USD Pyth Core sponsored push-feed fixture through canonical Solana RPC, verifies account/feed/full-verification/price/exponent/confidence/`publish_time`, applies a separate Pyth 60-second/5-second source policy, and can compare Jupiter/Pyth numerical prices plus exact fact-time delta. DEX Screener fact-time, Jupiter–Pyth time-identity policy, market-source independence, and current-price promotion remain false.
