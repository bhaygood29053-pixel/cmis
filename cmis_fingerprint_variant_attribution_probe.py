#!/usr/bin/env python3
"""Read-only CMIS v1.4.4 fingerprint-variant attribution probe."""
...
def main():
    ...
    variant_count = sum(
        int((item.get("summary") or {}).get(
            "variant_outlier_signature_count"
        ) or 0)
        for item in reports
    )
    all_ranges_proven = bool(reports) and all(
        (
            (item.get("baseline") or {}).get("range_proven") is True
            and (
                item.get("proof_scan") is None
                or (item.get("proof_scan") or {}).get(
                    "range_proven"
                ) is True
            )
        )
    ...
    result = {
        "service": "fingerprint_variant_attribution_probe",
        "version": "1.4.4",
        "chain": "x1",
        "asset": dict(resolved),
        "status": (
            "variant_outliers_attributed"
            if all_ranges_proven and variant_count > 0
        ...
        "summary": {
            ...
            "variant_outlier_signature_count": variant_count,
            "variant_outlier_observed": variant_count > 0,
            ...
            "interpretation": (
                "v1.4.4 identifies exact non-dominant fingerprint "
                "signatures and layout differences. It does not lower "
                "fingerprint thresholds and does not declare variants "
                "legitimate."
            ),
        },
        "errors": [],
