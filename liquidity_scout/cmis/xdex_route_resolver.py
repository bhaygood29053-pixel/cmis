...
def _validated_snapshot(snapshot: Any, requested_route: Mapping[str, Any]) -> dict[str, Any]:
    ...
    token_in_amount = _decimal(snapshot.get("token_in_amount"), "snapshot.token_in_amount")
    if token_in_amount <= 0:
        raise XDEXRouteResolverError("snapshot token_in_amount must be positive")
...
def _bounded_fee_capability(
    ...
    try:
        classified = classify_xdex_execution_fee_sequence_evidence(
...
def resolve_xdex_route_evidence(
    ...
    *,
    ...
    execution-slippage observation. Bounded fee evidence is also absent unless
    the accepted historical execution-evidence classifier is explicitly
    satisfied for this exact route and current config fee.
    """
