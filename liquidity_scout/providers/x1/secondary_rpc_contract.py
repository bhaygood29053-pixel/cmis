...
def classify_secondary_rpc_response(
    *, source: str, method: str, payload: Mapping[str, Any]
    ...
    error_code = None
    if isinstance(error, Mapping) and isinstance(error.get("code"), int) and not isinstance(error.get("code"), bool):
    ...
    if transport_ok:
        ...
        if method == "getHealth":
            ...
            result_ok = isinstance(result, int) and not isinstance(result, bool) and result >= 0
            if result_ok:
            ...
            if result_ok:
                ...
                if isinstance(parent_slot, int) and not isinstance(parent_slot, bool) and parent_slot >= 0:
