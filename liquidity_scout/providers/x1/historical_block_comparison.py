...
def extract_historical_block_fact(
    *, source: str, requested_slot: int, payload: Mapping[str, Any]
    ...
    """Extract an exact block identity from one already-observed getBlock response."""
...
def compare_historical_block_facts(
    ...
    """Compare one exact historical block identity across independent sources.

    Agreement proves only the identity fields of this one requested block. It
    never proves archival depth, retention, finality equivalence, or broader
    ...
    conflicts = tuple(
        field
        for field in fields
        if getattr(official, field) != getattr(secondary, field)
    )
