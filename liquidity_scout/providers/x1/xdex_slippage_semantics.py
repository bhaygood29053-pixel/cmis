...
class XdexSlippageSemanticResult:
    ...
    onchain_minimum_out_binding_verified: bool
...
def classify_xdex_slippage_semantics(
    *,
    ...
    """Classify the narrow quote-side slippage contract proven by observations.

    ``slippage_percent`` is interpreted only as the already-established XDEX quote
    request unit (percent, not a fraction). The expected raw output is:

        floor(zero_slippage_output_raw * (1 - slippage_percent / 100))

    This function deliberately does not claim that quote output is the eventual
    on-chain minimum amount out, nor that it predicts fill or execution quality.
    """
    ...
    return XdexSlippageSemanticResult(
        ...
        onchain_minimum_out_binding_verified=False,
