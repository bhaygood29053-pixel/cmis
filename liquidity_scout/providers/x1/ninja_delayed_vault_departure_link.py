...
DEFAULT_PATTERN_SUPPORT_MINIMUM = 3
DEFAULT_PATTERN_SUPPORT_MINIMUM_DISTINCT_POOLS = 2
    ...
    *,
    ...
    def inspect(
        ...
        *,
        ...
        referenced_accounts = set(accounts)
        referenced_accounts.update(parsed_endpoints)
        touched = sorted(exact_vaults.intersection(referenced_accounts))
        if not touched:
            return
        touches.append({
            "scope": scope,
    ...
    *,
    ...
    if classified.get("recognized_amm_invoked") is True:
        ...
        if (
            ...
            raise ValueError(
                attribution.get("warning")
                or "exact_vault_delta_attribution_unavailable"
            )
    else:
        attribution = None
    ...
    return {
        ...
        "exact_vault_delta_attribution": attribution,
        **classified,
    }
...
def aggregate_delayed_vault_departure_links(
    ...
    *,
    minimum_supported_departures: int = DEFAULT_PATTERN_SUPPORT_MINIMUM,
    minimum_supported_distinct_pools: int = (
        DEFAULT_PATTERN_SUPPORT_MINIMUM_DISTINCT_POOLS
    ),
) -> dict[str, Any]:
    """Aggregate bounded evidence while separating events from pattern claims.

    A verified delayed link is a per-event deterministic fact. It does not
    require a pattern-level sample threshold and may be surfaced as event-level
    evidence immediately.

    The bounded aggregate may additionally mark the pattern as supported after
    a predeclared diversity gate: at least three clean delayed links, spanning
    at least two distinct pools, with unique event keys, unique linked swaps,
    and zero complete counterexamples.

    departure_pattern_verified is intentionally reserved for a separate
    accumulated-evidence verifier. A bounded live sample never promotes that
    stronger longitudinal claim by itself.
    """

    if isinstance(minimum_supported_departures, bool) or not isinstance(
        minimum_supported_departures, int
    ):
        raise ValueError("minimum_supported_departures must be an integer")
    if minimum_supported_departures < DEFAULT_PATTERN_SUPPORT_MINIMUM:
        raise ValueError("minimum_supported_departures must be at least 3")

    if isinstance(minimum_supported_distinct_pools, bool) or not isinstance(
        minimum_supported_distinct_pools, int
    ):
        raise ValueError(
            "minimum_supported_distinct_pools must be an integer"
        )
    if (
        minimum_supported_distinct_pools
        < DEFAULT_PATTERN_SUPPORT_MINIMUM_DISTINCT_POOLS
    ):
        raise ValueError(
            "minimum_supported_distinct_pools must be at least 2"
        )
    if minimum_supported_distinct_pools > minimum_supported_departures:
        raise ValueError(
            "minimum_supported_distinct_pools cannot exceed "
            "minimum_supported_departures"
        )
    ...
    unique_events = bool(
        departures
        and len(keys) == len(departures) == len(set(keys))
    )
    unique_swaps = bool(
        delayed
        and len(signatures) == len(delayed) == len(set(signatures))
    )
    delayed_pool_identity_complete = bool(
        delayed
        and len(delayed_pools) == len(delayed)
    )
    distinct_delayed_pools = len(set(delayed_pools))

    complete_counterexample_count = (
        counts[LATEST_SWAP_MATCHES_BEFORE]
        + counts[LATEST_SWAP_MATCHES_NEITHER]
    )
    incomplete_or_ambiguous_count = (
        counts[UNAVAILABLE_OR_INCOMPLETE]
        + counts[SAME_SLOT_AMBIGUITY]
    )
    no_complete_counterexample = complete_counterexample_count == 0

    event_level_evidence_available = bool(delayed)
    pattern_supported = bool(
        len(delayed) >= minimum_supported_departures
        and distinct_delayed_pools >= minimum_supported_distinct_pools
        and delayed_pool_identity_complete
        and unique_events
        and unique_swaps
    ...
    if pattern_supported:
        evidence_maturity = "pattern_supported"
    elif event_level_evidence_available:
        evidence_maturity = "event_verified"
    elif rows:
        evidence_maturity = "observed_no_verified_link"
    else:
    ...
    return {
        ...
        "version": VERSION,
        "chain": "x1",
        "status": "verified" if pattern_supported else (
            "partial" if rows else "unavailable"
        ),
        "evidence_maturity": evidence_maturity,
        "event_count": len(rows),
        "verified_departure_count": len(departures),
        "minimum_supported_departures": minimum_supported_departures,
        "minimum_supported_distinct_pools": (
            minimum_supported_distinct_pools
        ),
        ...
        "distinct_delayed_pool_count": distinct_delayed_pools,
        "delayed_pool_identity_complete": delayed_pool_identity_complete,
        "complete_counterexample_count": complete_counterexample_count,
        "incomplete_or_ambiguous_count": incomplete_or_ambiguous_count,
        "outcome_counts": counts,
        "price_only_reserve_ratio_departure_verified": bool(departures),
        "delayed_vault_swap_execution_link_verified": (
            event_level_evidence_available
        ),
        "event_level_evidence_available": event_level_evidence_available,
        "departure_pattern_supported": pattern_supported,
        ...
        "departure_pattern_verified": False,
        "pattern_verification_basis": "accumulated_evidence_required",
        "pattern_verification_from_bounded_sample_authorized": False,
        "provider_fact_time_verified": False,
        "update_source_semantics_verified": False,
        "freshness_verified": False,
        "universal_catalog_price_semantics_verified": False,
