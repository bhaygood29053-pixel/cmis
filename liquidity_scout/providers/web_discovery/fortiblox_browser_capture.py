"""Operator-controlled passive FortiBlox App browser/network capture.

Playwright is optional and imported lazily. The capture opens exactly one
explicit same-host FortiBlox App page in a fresh ephemeral browser context,
observes bounded network responses, immediately sanitizes eligible traffic
through fortiblox_network_observation/v1, and returns only sanitized records.

No clicks, forms, wallet interaction, authentication, payment, request replay,
persistent browser profile, raw HAR retention, or execution authority exists.
"""
...
BROWSER_CAPTURE_CONTRACT = "fortiblox_browser_capture/v1"
...
def _load_sync_playwright() -> Callable[[], Any]:
    try:
        ...
        raise RuntimeError(
            "FortiBlox browser capture requires the optional Playwright "
...
def _transient_har_entry(response: Any) -> dict[str, Any] | None:
    ...
    if not _allowed_target(request_url):
        return None

    route = classify_fortiblox_network_route(method, request_url)
    ...
    if method == "POST":
        ...
        if not isinstance(post_data, str) or not post_data.strip():
            return None
        if len(post_data.encode("utf-8")) > MAX_REQUEST_BODY_BYTES:
            return None
...
def _single_entry_har(entry: Mapping[str, Any]) -> dict[str, Any]:
    return {"log": {"version": "1.2", "entries": [dict(entry)]}}


def capture_fortiblox_page_network(
    page_url: str,
    *,
    ...
    """Passively capture one explicit FortiBlox App page."""
    ...
    with factory() as playwright:
        ...
        try:
            ...
            def on_response(response: Any) -> None:
                ...
                sanitized = list_fortiblox_network_observations(
