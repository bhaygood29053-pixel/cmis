...
``python -m liquidity_scout.integrations.moltgrid``, the three listener-facing
ranking operations dynamically resolve to CMIS-backed adapters instead.  This
keeps shared calculation code pure while making the live MoltGrid path exercise
the same ``rank`` gateway contract future Scouts consume.
"""
...
def __getattr__(name):
    if name not in {"find_asset_rank", "format_top", "ranking_row"}:
