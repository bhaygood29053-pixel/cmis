"""
Liquidity Scout v0.3 — Dynamic XDEX Signal Listener
...
def find_unanswered_messages(posts, catalog, implicit_mode_started_at):
    ...
    for post in posts:
        ...
        if is_reply_to_bot:
            ...
            # Reuse the resolution so we don't search the catalog twice.
            incoming.append(
                (post, "standalone-implicit", resolved_term, resolved_matches)
            )
...
def requested_asset_fields(question):
    ...
    if any(x in q for x in ("price", "worth", "trading at", "cost")):
        add("price")
    if "age" in q or "how old" in q or "created" in q or "launched" in q:
        add("age")
    if "holder" in q:
        add("holders")
    if any(x in q for x in ("transaction", "transactions", "txns", "txn")):
        add("txns24")
    if "volume" in q and not wants_volume_rank(question):
        add("volume24")

    one_hour = ("1h", "1 h", "1hr", "1 hr", "1 hour", "one hour", "hourly")
    day_24 = ("24h", "24 h", "24hr", "24 hr", "24 hour", "24-hour", "daily")
    change_words = ("change", "move", "performance", "how much up", "how much down")
    has_change = any(x in q for x in change_words)

    if has_change and any(x in q for x in one_hour):
        add("change1h")
    if has_change and any(x in q for x in day_24):
        add("change24h")
    if has_change and "change1h" not in fields and "change24h" not in fields:
        add("change1h")
        add("change24h")

    if "liquidity" in q or " liq " in f" {q} ":
        add("liquidity")
    if any(x in q for x in ("market cap", "marketcap", "mcap")):
        add("market_cap")
    if "safety" in q or "safe" in q:
        add("safety")
...
def format_pool_answer(question, term, matches, catalog):
    ...
    if len(matches) > 1:
        lines.append(
            ...
            f"{snap['pool']}, the strongest match by liquidity/volume."
        )
    else:
        lines.append(f"Pool: {snap['pool']}.")
...
def process_cycle(catalog, implicit_mode_started_at):
    # Refresh first because implicit standalone detection may need to resolve
    # an asset before deciding whether a Signal is meant for this bot.
    ...
    for post, message_type, pre_term, pre_matches in pending[:5]:
        ...
        # A standalone Signal becomes the parent of the answer.
        # A reply to an existing Liquidity Scout Signal gets the answer attached
        # to the original parent Signal for MoltGrid UI visibility.
        ...
        if len(multi) >= 2:
            ...
            print(f"Multiple XDEX assets detected: {names}")
            ...
            if matches:
                print(f"XDEX asset detected: {term}")
                answer = format_pool_answer(question, term, matches, catalog)
            else:
                print("No XDEX asset detected.")
                answer = format_not_found(question)
...
def main():
    ...
    print("Liquidity Scout v0.10 — Intent-Aware XDEX Signal Listener")
    ...
    print("Input: replies + explicit new Signals + owner XDEX asset questions")
