...
| X1-BRIDGE-04 | Bridge transfer state / history | PARTIAL / EVENTS + COUNTER CLOSURE VERIFIED | #433 preserves the official wallet-history endpoint and independently pairs exact on-chain OutgoingMsg/IncomingMsg state into real settled events for wSOL <-> wSOL.X. #437 verifies official config counters == on-chain Config counters == unique current message-account counts on both chains, with observed message timestamps extending beyond 60 days. Historical deletion/recycling semantics remain unverified, so retention/window coverage is not yet promoted and missing periods never become zero. |
