...
Warp Bridge exact route/config semantics are accepted for provenance-qualified routes through #407 / PR #429. Under #409, the deterministic flow calculator is merged and Issue #433 / PR #434 verifies real settled wSOL-route events by exact on-chain OutgoingMsg/IncomingMsg pairing. Historical retention completeness and bridged-supply semantics remain unverified, so no 24h/7d/30d total or supply value is promoted yet.
...
| X1-BRIDGE-04 | Bridge transfer state / history | PARTIAL / REAL SETTLED EVENTS VERIFIED | #433 / PR #434 pairs exact on-chain OutgoingMsg/IncomingMsg state into real settled events for wSOL <-> wSOL.X. Current account-state enumeration is verified; historical retention/coverage completeness remains unverified, so missing periods never become zero. |
...
5. **Warp Bridge / #409** — route/config semantics and real on-chain settled-event pairing are accepted foundations. Next prove historical retention/coverage for the requested flow windows and a separate current bridged-supply semantic contract; until then window totals/supply remain unavailable.
