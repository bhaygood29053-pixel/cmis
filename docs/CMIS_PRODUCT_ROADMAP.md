...
- **CROSS-CHAIN / WARP: ACTIVE NEXT EVIDENCE GATE.** Cross-chain provenance and bridge-route/Warp qualification foundations are accepted internally, but Warp remains `blocked_endpoint_semantics`. CMIS Issue #407 is the next exact endpoint/semantic-fixture gate. PR #412 has already merged the deterministic `warp_machine_contract_capture/v1` harness; the current HAR-ingestion slice only makes official-app network evidence easier to acquire and does not accept any endpoint semantics. No bridge truth is promoted to Scout/ROBERTA yet.
