...
- **CROSS-CHAIN / WARP: #409 ACTIVE — REAL SETTLED EVENTS + COUNTER CLOSURE VERIFIED; RETENTION STILL GATED.** CMIS Issue #407 / PR #429 accepted `warp_config/exact-mint-pair/v1` for exact provenance-qualified config routes, including wSOL -> wSOL.X. PR #432 merged the deterministic `bridge_flow_intelligence/v1` calculator. Issue #433 / PR #435 + PR #436 preserve the official History-page wallet endpoint and independently pair exact on-chain `OutgoingMsg`/`IncomingMsg` state into real settled events; exact-head live evidence produced 16/16 paired wSOL-route events with zero unresolved mismatches and fed all 16 into the calculator. Issue #437 / PR #438 adds `warp_message_retention_coverage/v1`: live official config counters equal the independently decoded on-chain Config counters and the complete verified message-account counts on all four axes (Solana out=3967/in=6274; X1 out=6279/in=3994), with unique sequence counts equal to account counts. The observed message universe starts at Unix `1765954570` and therefore spans beyond the 60-day #409 requirement. However, counter closure does not by itself prove that historical `OutgoingMsg`/`IncomingMsg` accounts cannot be deleted or recycled, so `historical_retention_complete_verified=false`, `coverage_complete_verified=false`, and 24h/7d/30d totals remain `null`, never zero. Bridged supply remains unavailable pending a separate accepted supply semantic contract. `execution_authorized=false`.
