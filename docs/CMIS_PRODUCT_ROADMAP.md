...
- **CMIS Web Discovery v1-v10: COMPLETE internally through PR #495.** X1.Ninja route reconciliation now records all five documented Developer API routes covered by v9, zero known documented route gaps, and `browser_capture_required_now=false`. **Issue #496 is the active v11 X1.Ninja semantic-coverage reconciliation**, classifying 13 semantic families as exactly `verified`, `partial`, `blocked`, or `unavailable`; it keeps #461/PR #470 liquidity USD semantics and #459 freshness blocked while preserving bounded #341 reserve and PR #465 fact-time proofs.
...
- **CMIS capability contract: `1.21.0`.** Burn Intelligence (`burn_intelligence/v1`) remains accepted under 1.15.0, Discovery Intelligence (`discovery_intelligence/v1`) under 1.16.0, field-scoped current-market freshness under 1.17.0, and pull-only Concentration Warning Intelligence (`concentration_warning_intelligence/v1`) under 1.18.0.
- **Instant X1 Scan: COMPLETE through `instant_x1_scan/v4`.** v4 accepts exact `x1_current_market_freshness/v2` so accepted liquidity and rolling 24h volume/transaction evidence can be surfaced field-by-field without promoting provider fact-time or source independence. v3 remains backward compatible.
