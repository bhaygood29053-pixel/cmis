...
- **CROSS-CHAIN / WARP: #409 ACTIVE — REAL SETTLED EVENTS VERIFIED, COVERAGE STILL GATED.** CMIS Issue #407 / PR #429 accepted `warp_config/exact-mint-pair/v1` for exact provenance-qualified config routes, including wSOL -> wSOL.X. PR #432 merged the deterministic `bridge_flow_intelligence/v1` calculator. Issue #433 / PR #434 now establishes a read-only real-event source by pairing on-chain Warp `OutgoingMsg` and `IncomingMsg` accounts using exact Anchor discriminator, account length, PDA, sequence, sender, amount, source timestamp, exact route mints, operation topology, processed state, and execution timestamp. Exact-head live evidence paired all 16 observed wSOL-route candidates into 16 settled events with zero unresolved mismatches and fed all 16 into the #409 calculator. The API `/transactions` surface remains non-authoritative for complete history because page-2 pagination was inconsistent and executed `/signatures` returned no message identity. Historical retention/coverage remains unverified, so 24h/7d/30d totals stay `null` rather than zero. Bridged supply also remains unavailable pending a separate accepted supply semantic contract. `execution_authorized=false`.
