...
9. **X1 Intelligence Brief runtime promotion (#637): PUBLIC CONTRACT STACK ACCEPTED / PROTECTED REGISTRATION BLOCKED ON #52.** Accepted public pieces are the bounded composer (#635 / PR #636), caller-selector-only `x1_intelligence_brief_request/v1` (PR #639), and the non-promoted standard CMIS service-envelope projection (PR #641 / `d65c545179284b36f57e7341a0238e639fa40b4a`). Protected internal matrix construction is prepared in `cmis-core` PR #53, including canonical service-only component calls and no invented Concentration Warning threshold, but remains unmerged because private Actions still fail before recording steps. Remaining work is executable protected CI, protected registration, deliberate CMIS contract/capability promotion, and explicit X1 Scout reliance. No whole-X1 coverage claim and no execution authority.
