...
- **X1 Intelligence Brief inputs: ACTIVE FOUNDATION under #635 / PR #636.** The current work is contract-only and non-promoted: exact X1 mint subjects, <=24h `[start,end)` windows, explicit subject×service coverage, deterministic priority separate from risk, exact duplicate collapse, and no whole-X1 coverage claim. Initial fact-time-safe inputs are Concentration Warning, Large-Trade Discovery, and Discovery Intelligence. ROBERTA #412 remains downstream and blocked on later explicit promotion.
...
9. **Bounded X1 Intelligence Brief inputs (#635 / PR #636): ACTIVE PARALLEL PRODUCT FOUNDATION.** Compose only accepted CMIS responses with service-specific verified fact time; do not substitute envelope collection time for event time. This track is independent of the private-runner blocker on #631. Foundation flags remain `public_service_promoted=false`, `scout_reliance_promoted=false`, and `execution_authorized=false`.
