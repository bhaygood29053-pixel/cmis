...
- **CROSS-CHAIN / WARP: ACTIVE NEXT EVIDENCE GATE.** Cross-chain provenance and bridge-route/Warp qualification foundations are accepted internally, but Warp remains `blocked_endpoint_semantics`. CMIS Issue #407 is the next exact endpoint/semantic-fixture gate. PR #412 has already merged the deterministic `warp_machine_contract_capture/v1` harness; the HAR-ingestion slice only makes official-app network evidence easier to acquire and does not accept any endpoint semantics. No bridge truth is promoted to Scout/ROBERTA yet.
- **WARP ON-CHAIN FALLBACK: ACCEPTED THROUGH RARE RAW CAPTURE.** Issue #419 / PR #423 established the zero-byte Warp-owned account inventory. Issue #425 / PR #426 are now merged on `main` under `warp_rare_account_capture/v1`: the 170/236/321/335-byte rare families were re-fetched through bounded read-only `getAccountInfo(base64)`, exact owner/length/non-executable state verified, full-byte SHA-256 recorded ephemerally, and bounded prefix/suffix evidence retained without committing raw bytes. Four exact cross-chain rare-account pubkey overlaps were observed; the 236-byte account was byte-identical across chains. Semantic promotion remains false and `execution_authorized=false`.\n- **WARP SEMANTIC LAYOUT: ACTIVE NEXT SLICE.** Issue #428 advances #407 with `warp_semantic_layout_discovery/v1`. A pinned public third-party Warp IDL is used only as corroborating evidence, then account type requires exact live size + Anchor discriminator + reproducible PDA identity. Current reproducible mapping is 170=`TokenRegistryEntry`, 236=`Roles`, 321=`Config`, 335=`GuardianSet`; fixed PDA seeds reproduce the observed Config/GuardianSet/Roles singleton pubkeys. The live gate must still keep `field_semantics_verified=false`, `semantic_contract_accepted=false`, `cmis_promotable=false`, and `execution_authorized=false` until specific fields are independently corroborated.
