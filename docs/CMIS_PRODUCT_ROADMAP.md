...
- **#410 Bridge-to-XDEX Utilization Intelligence: COMPLETE.** PR #469 remains the accepted bounded 24h XDEX-program activity/value-basis proof.
- **#482 Bridge-to-XDEX public-service / Scout promotion: COMPLETE.** Public PR #487 and protected `cmis-core` PR #23 are merged. `bridge_to_xdex_utilization/v1` is promoted under CMIS 1.19 with the original route/mint/program-family/value-basis guardrails preserved.
- **#491 cross-chain asset provenance promotion: COMPLETE.** Public PR #493 and protected `cmis-core` PR #24 are merged. `cross_chain_asset_provenance/v1` is promoted for X1 Scout reliance under CMIS 1.20 without turning provenance into backing, solvency, safety, adoption, causality, current bridge state, or risk.
- **#461 X1.Ninja USD-liquidity semantics: COMPLETE.** PR #470 is merged and Issue #461 is closed. The accepted five-pool proof establishes the bounded USD-liquidity semantic basis; it does **not** by itself establish current liquidity freshness.
- **#459 field-scoped freshness expansion: ACTIVE.** PR #500 is the current implementation candidate for `x1_current_market_freshness/v2`. Its first dedicated liquidity-freshness run and the standard public-shell suite are currently failing at deterministic aggregate-liquidity-freshness coverage, so PR #500 is **not accepted**. Rolling 24h volume and rolling transaction freshness remain unverified.
- **CMIS Web Discovery v1-v11: COMPLETE internally through PR #497.** X1 Explorer, XDEX, and X1.Ninja discovery/reconciliation slices are present as bounded discovery only; they do not independently create verified market truth or Scout authority.
- **#498 Verified Trade Attribution + Pool Price Impact: ACTIVE / PLANNED.** The proposed `trade_price_impact_intelligence/v1` remains read-only and must keep pool-local AMM effects separate from whole-market causality, wallet identity/intent, manipulation labels, automatic risk, recommendation, or execution.
- **Current accepted CMIS capability contract: `1.20.0`.**
...
- **CMIS capability contract: `1.20.0`.** Burn Intelligence (`burn_intelligence/v1`) remains accepted under 1.15.0, Discovery Intelligence (`discovery_intelligence/v1`) under 1.16.0, field-scoped current-market freshness v1 under 1.17.0, pull-only Concentration Warning Intelligence (`concentration_warning_intelligence/v1`) under 1.18.0, Bridge-to-XDEX utilization promotion under 1.19.0, and cross-chain asset provenance promotion under 1.20.0.
