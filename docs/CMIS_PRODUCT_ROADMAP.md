...
- **CMIS Web Discovery v1-v6: COMPLETE internally.** PRs #472/#474/#476/#478 established the X1 Explorer/multi-source discovery stack; PR #481 added structured XDEX endpoint discovery; PR #484 added the deterministic XDEX network-gap registry and accepted `browser_capture_required_now=false` for the known gap set. **Issue #485 is the active v7 XDEX extended read-only structured-discovery slice**, covering the frontend quote alias plus XDEX Oracle token-price and sell-quote GET surfaces while preserving all semantic, source-independence, promotion, and execution boundaries.
