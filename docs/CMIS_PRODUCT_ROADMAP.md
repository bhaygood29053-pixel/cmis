...
- **X1 Intelligence Brief inputs v1 foundation: ACCEPTED.** #635 is complete through merged PR #636 (`610f7f04f557bf12e44453f38e86212a65803089`). The accepted contract remains non-promoted and composes only Concentration Warning, Large-Trade Discovery, and Discovery Intelligence with service-specific fact time, <=24h `[start,end)` windows, explicit subject×service coverage, deterministic priority separate from risk, duplicate safety, and no whole-X1 coverage claim. Runtime/Scout promotion is now #637 and is blocked on protected Actions recovery #52.
- **FortiBlox price fact-time gate: CLOSED EVIDENCE_INCOMPLETE.** PR #648 merged as `693e24bfa9385ff1bc24cfa883a34ede788572ae`. The accepted `fortiblox_price_fact_time_probe/v1` distinguishes PROVEN / DISPROVEN / EVIDENCE_INCOMPLETE without promoting freshness. The bounded live no-payment gate observed HTTP 402 on all six `/api/tokens` attempts, so no field-level `updatedAt`→`priceUsd` proof exists. `fortiblox_price_freshness/v1` remains uncreated; current-fact corroboration, source independence, volume freshness, public/Scout promotion, and execution remain false.
...
- **Large-Trade Discovery: ACCEPTED.** Public PRs #532/#533 and protected `cmis-core` PR #35 establish promoted `large_trade_discovery/v1` under CMIS 1.25. The accepted service is provider-scoped and does not claim every X1 DEX or real-world wallet identity.
- **Large-Trade → #498 live handoff: ACCEPTED.** Protected `cmis-core` PR #41 passed its exact dedicated live workflow on run #7 at head `9ff63bcac15d9bd7f46868489f444508ed126c06`, then merged as `f659f53f3d565bd5886dfae3e1a12370100cddc9`; Issue #40 closed completed. The accepted path proves bounded provider-scoped Large-Trade ranking → protected #498 materialization/storage → rebuilt ready wallet/price-impact handoff without widening execution, ownership, causality, risk, or global-DEX claims.
...
1. **Provider-gap verification (#30): ACTIVE umbrella.** Continue read-only provider qualification only where it closes a real X1 evidence gap.
...
4. **Early Warning: COMPLETE THROUGH PULL-ONLY ROBERTA ADOPTION.** Push/delivery remains a separate future product gate and is not implied by the existing warning contract.
5. **Warp retention #437: COMPLETE / CLOSED. Theo transport #422: EXTERNALLY BLOCKED. Delayed-departure longitudinal verification #381: EVIDENCE-WAITING.** #437 is reconciled complete because its remaining lifecycle-retention requirement moved to child #441, which closed completed after merged PR #453 (`dd86a0443f195183d57723cd99f0a5a325d03888`) implemented the accepted bounded lifecycle-retention proof. Theo #422 remains open but is not an active software blocker: X mention, X API, xChat, and Telegram tests have not produced an acceptable current machine transport contract; keep `ACCEPTED_THEO_TRANSPORT_CONTRACTS` empty until provider-side identity + transport evidence exists. #381 remains evidence-waiting until a predeclared longitudinal replication rule can be evaluated without cherry-picking. None of these blocks the accepted flagship scan, wallet/trade, cross-chain, regulatory, or warning surfaces.
...
9. **X1 Intelligence Brief runtime promotion (#637): FINAL PROMOTION STACK STAGED / BLOCKED ONLY ON PRIVATE ACTIONS #52.** Accepted public foundations are the bounded composer (#635 / PR #636), caller-selector-only `x1_intelligence_brief_request/v1` (PR #639), and the standard service-envelope foundation (PR #641). Protected matrix construction is prepared in `cmis-core` PR #53, the protected selector-only dispatch adapter in draft PR #54, and final `RuntimeCMISGateway` / protected `SUPPORTED_SERVICES` registration in draft PR #55. Public draft PR #643 stages deliberate CMIS 1.28 capability promotion and is green on Liquidity Scout Tests #1884 at exact head `536cabbc9d9fe8a96f1f24063a59bdd9fb00e780`. The accepted nested `x1_intelligence_brief_inputs/v1` factual payload remains non-promoted while the outer service envelope/capability carries promoted read-only runtime state; no fact contract is silently mutated. None of #53/#54/#55/#643 may merge until private Actions #52 executes real package 3.11/3.12 + public-shell compatibility steps and passes. No whole-X1 coverage claim and no execution authority.
...
- **#507 Instant X1 Scan v4 promotion: COMPLETE.** Public PR #508 and protected `cmis-core` PR #26 are merged. CMIS 1.21 exposes `instant_x1_scan/v4` over exact `x1_current_market_freshness/v2`; v3 remains backward compatible. Missing runtime-owned evidence still fails closed, and provider fact-time/source independence remain separate.
...
- **CMIS capability contract: `1.27.0`.** Current promoted additions include `trade_price_impact_intelligence/v1`, `large_trade_discovery/v1`, and freshness-aware `regulatory_evidence/v1`; earlier accepted Burn, Discovery, concentration-warning, cross-chain, history, identity, and scan surfaces remain available under their original authority boundaries.
- **Instant X1 Scan: COMPLETE through `instant_x1_scan/v6`.** v6 is the current accepted product composition and preserves exact fail-closed field, distribution, concentration, history-adequacy, and source-scope semantics.
...
2. **X1.Ninja developer API validation — FOUNDATION / FOLLOW-ON SEMANTICS.** The original transport/shape and exact-pool verification program produced later accepted evidence foundations. Remaining unit, field-semantics, freshness, delayed-update, and source-independence questions stay fail-closed and are follow-on work only when tied to an active product gap; they are no longer the blanket default next roadmap gate.
...
7. **Early Warning services — COMPLETE THROUGH PULL-ONLY PROMOTION.** Persistence, freshness, identity, replay/deduplication, warning-state semantics, public CMIS promotion, and ROBERTA pull-only adoption are accepted. Push/delivery remains a separately gated future capability.
...
## Live reconciliation — 2026-09-05 America/New_York
...
- **CMIS Web Discovery v1-v10: COMPLETE internally through PR #495; #496 is the active X1.Ninja semantic-coverage reconciliation gate.**
