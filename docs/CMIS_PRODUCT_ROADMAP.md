...
- **CMIS Web Discovery v1-v5: COMPLETE internally.** PRs #472/#474/#476/#478 established the X1 Explorer/multi-source discovery stack; PR #481 added structured XDEX endpoint discovery and deterministic handoff back into the existing XDEX provider/verification path. **Issue #483 is the active v6 XDEX network-gap registry**, classifying covered read-only surfaces, direct machine-readable gaps, execution-adjacent exclusions, and UI-only candidates. The current required decision is `browser_capture_required_now=false` for known XDEX gaps.
