...
- **CMIS Web Discovery: COMPLETE internally through the accepted source-specific stack, including X1 Agents Radio.** Radio source discovery is accepted via #564 / PR #566, `x1_agents_radio_structured_discovery/v1` via #568 / PR #569, and direct-chain `x1_agents_radio_rpc_corroboration/v1` via #571 / PR #572. Canonical finalized X1 RPC may verify exact current account existence/executable/owner state, bounded history, and successful exact-slot transaction activity. Radio names, categories, frameworks, instruction semantics, 24h counts, and deployment-vs-upgrade semantics remain unverified; no public-service or Scout-reliance promotion is implied.
