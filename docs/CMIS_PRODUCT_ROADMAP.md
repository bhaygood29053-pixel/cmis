...
- **XONE/XNT Conversion Intelligence: ACCEPTED INTERNAL FOUNDATION via #575 / PR #578.** Dedicated `xone_xnt_conversion_scraper/v1` and `xone_xnt_conversion_intelligence/v1` remain separate from generic Web Discovery and collect XONE-focused candidate claims from bounded X1/X1 Docs/X1 Report sources. Web claims, Ethereum events, X1 events, and Ethereum→X1 correlation remain unverified until direct-chain corroboration; the exact Ethereum XONE contract identity is intentionally not hardcoded yet. No public-service/Scout promotion or execution authority is implied.
