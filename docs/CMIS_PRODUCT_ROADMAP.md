...
- **XONE/XNT Conversion Intelligence: PRIMARY TRACK REMAINS THE ETHEREUM XONE SNAPSHOT / REGISTRY; BOUNDED PUBLIC ALLOCATION-SOURCE PROVENANCE IS NOW COMPLETE.** The accepted chain now extends through #619 / PR #620 `xone_xnt_allocation_source_provenance/v1`, after #616 / PR #617 completed X1-side allocation-record discovery. Final provenance run #6 covered 3/3 repositories, 41 provenance-shaped repository files, 3/3 release queries, and 3/3 official X1 targets. It retained 10 clean `x1_program_schema` architecture analogues from `x1-labs/xenblocks-airdrop`, but found **0 qualifying XONE allocation-source candidates and 0 exact-XONE-bound sources**. No XONE-linked API, Merkle source, IPFS/Arweave artifact, registry/export, release asset, structured holder-allocation file, or exact XONE-bound X1 program schema surfaced in the bounded public corpus. Earlier live cycles exposed and permanently regression-tested source-id semantic leakage, generic TypeScript `export`/config noise, generic RPC URLs, the `pda` substring inside `update`, and IDL dual-classification. This is scoped public-source evidence only: `private_or_unpublished_allocation_source_absence_proven=false`. The next work is source-specific primary-artifact resolution or evidence intake only when a new exact registry/export/API/Merkle/deployment/program lead appears; broad current-web/archive/repository discovery is no longer the highest-value path. Snapshot→XNT allocation binding, XNT issuance/vesting/unlock, October 6 applicability, cross-chain correlation, public/Scout promotion, and execution remain unverified/unauthorized.
