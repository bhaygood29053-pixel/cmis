...
9. **X1 Intelligence Brief runtime promotion (#637): PUBLIC REQUEST CONTRACT ACCEPTED / PROTECTED RUNTIME BLOCKED ON #52.** The #635/PR #636 composition foundation is accepted, and PR #639 adds the accepted caller-selector-only `x1_intelligence_brief_request/v1` request contract. Callers may select exact X1 mint subjects, bounded UTC window, and accepted component classes only; they cannot inject component facts/evidence/priority/fact time/risk/provider state. Remaining work is the protected internal subject×service matrix builder, exact-head private CI, deliberate CMIS contract/capability promotion, and explicit X1 Scout reliance. No whole-X1 coverage claim and no execution authority.
