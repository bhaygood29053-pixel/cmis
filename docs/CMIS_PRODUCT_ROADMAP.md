...
- **X1 Explorer Network Observation v1 — Issue #475 / PR #476: COMPLETE as an internal source-specific layer.** Sanitized browser-exported observations now recognize only allowlisted read-only X1 JSON-RPC methods and eligible JSON GETs, preserve bounded identifiers/hashes, reject credential-like material and execution-oriented RPC methods, never retain raw secret-bearing browser records, and remain `DISCOVERED` / non-promoted / `execution_authorized=false`.
- **Issue #477 — X1 Explorer Browser Capture v1: ACTIVE.** The next Web Discovery slice adds an operator-controlled, one-page passive Chromium capture utility with lazy optional Playwright loading. It immediately routes eligible network events through the accepted sanitizer, retains no raw HAR/profile/body material, performs zero clicks/forms/wallet actions, never replays requests, and keeps background monitoring, public-service/Scout promotion, and execution authority false.
...
- **CROSS-CHAIN / WARP: #409 COMPLETE; #410 ACTIVE.** CMIS Issue #407 / PR #429 accepted `warp_config/exact-mint-pair/v1`; PR #432 merged the deterministic `bridge_flow_intelligence/v1` calculator; PR #436 accepted canonical settled Warp events; PR #453 completed the bounded 60-day lifecycle-retention proof from #441; PR #452 accepted exact current bridged-supply closure; and PR #455 bound the accepted route, settled-event, retention, supply, and current/prior 24h/7d/30d flow evidence into the final #409 integration. Issue #409 is closed complete. **Issue #410 Bridge-to-XDEX Utilization Intelligence is now the active cross-chain implementation gate.** Its first slice must resolve exact wSOL.X XDEX pools, verified liquidity/24h volume with fact time, and a comparable value basis before any bridge-to-XDEX ratio is computed. Public-service / Scout promotion remains separate, and ROBERTA #314 stays blocked until #410 plus the required promotion contract are accepted. `execution_authorized=false`.
