...
- **CROSS-CHAIN / WARP: #409 ACTIVE — REAL SETTLED EVENTS VERIFIED, COVERAGE STILL GATED.** CMIS Issue #407 / PR #429 accepted `warp_config/exact-mint-pair/v1` for exact provenance-qualified config routes, including wSOL -> wSOL.X. PR #432 merged the deterministic `bridge_flow_intelligence/v1` calculator. Issue #433 now has two complementary evidence paths: PR #435 preserves the exact official History-page wallet endpoint pattern `GET /api/bridge/transactions/wallet/{wallet}?limit=100` from repeated GET+200+JSON observations with the wallet redacted, while the current on-chain source slice pairs Warp `OutgoingMsg` and `IncomingMsg` accounts by exact owner, discriminator, size, PDA, sequence, sender, amount, source timestamp, exact route mints, operation topology, processed state, and execution timestamp. The exact connected-wallet `/history` response body is also pinned as `warp_wallet_history_semantics/v1`: it confirms observed `executed` and `signing` states plus destination-reference fields, but remains wallet-scoped corroboration only; its token label, amount unit, provider timestamp, and status do not replace exact-mint/on-chain settlement truth. Exact-head live evidence paired all 16 observed wSOL-route candidates into 16 settled events with zero unresolved mismatches and fed all 16 into the #409 calculator. The separate public API `/transactions` discovery also showed why coverage must remain fail-closed: page 1 reported 61 executed rows while page 2 returned an empty `total=0`, and executed `/signatures` exposed no message identity. Historical retention/coverage remains unverified, so 24h/7d/30d totals stay `null`, never zero. Bridged supply remains unavailable pending a separate accepted supply semantic contract. `execution_authorized=false`. Issue #437 / PR #440 has now accepted `warp_message_retention_coverage/v1` current-universe counter/account closure by comparing official config counters, exact on-chain Config counters, and the full PDA-verified OutgoingMsg/IncomingMsg universes. **Issue #441 is now the active #409 coverage gate**: prove the required 60-day message-account lifecycle retention and rule out history loss through close/deletion/recycling over the requested lookback. Until #441 passes, historical retention, requested-window coverage, coverage completeness, and missing-history-as-zero authorization remain false.
- **WARP ON-CHAIN CORROBORATION: RARE CAPTURE ACCEPTED; LAYOUT HARDENING ACTIVE.** Issue #419 / PR #423 established the zero-byte Warp-owned inventory. Issue #425 / PR #426 then captured the 170/236/321/335-byte rare families through bounded read-only `getAccountInfo(base64)` with exact owner/length/non-executable verification and deterministic full-byte hashes; raw bytes remain ephemeral. Issue #428 independently corroborates account-type identity through exact size + Anchor discriminator + reproducible PDA, with the current evidence mapping 170=`TokenRegistryEntry`, 236=`Roles`, 321=`Config`, and 335=`GuardianSet`. This hardening does not replace the accepted official-config semantic contract from #407 and does not block #409; on-chain field semantics remain separately fail-closed and `execution_authorized=false`.
- **FORTISWAP PROVIDER: ACCEPTED READ-ONLY FOUNDATION.** Issue #413 / PR #414 are complete on `main`. CMIS now has bounded FortiSwap machine-discovery, token, token-detail, router-volume, and quote normalization with exact provider-host qualification and deterministic CI. FortiSwap assertions are not CMIS truth and remain separate from the accepted Warp exact-config route semantics; `execution_authorized=false`.
