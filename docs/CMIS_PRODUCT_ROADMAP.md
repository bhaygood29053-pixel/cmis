...
- **WARP ON-CHAIN FALLBACK: ACTIVE DISCOVERY.** Issue #419 inventories exact Warp-program-owned accounts on Solana and X1 through zero-byte `getProgramAccounts` reads. This is an independently verifiable alternative path into #407 if private API paths remain unpublished; account role/layout/route semantics remain unverified and `execution_authorized=false`.
