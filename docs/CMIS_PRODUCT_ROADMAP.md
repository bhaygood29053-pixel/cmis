...
- **XONE/XNT Conversion Intelligence: ACCEPTED INTERNAL FOUNDATION via #575 / PR #578; exact Ethereum XONE identity accepted via #580 / PR #581.** Dedicated `xone_xnt_conversion_scraper/v1` and `xone_xnt_conversion_intelligence/v1` remain separate from generic Web Discovery. Narrow `ethereum_xone_identity/v1` now verifies Ethereum mainnet XONE as `0x4DCDa2274899d9BbA3Bb6f5A852C107Dd6E4fE1c`; live run #6 proved the exact creation transaction/deployer, runtime code, and ERC-20 `XONE`/`XONE`/18 metadata across multiple public Ethereum RPC transports. Conversion rules, burns/migration deposits, eligibility, XNT issuance/vesting, and Ethereum→X1 correlation remain unverified. No public-service/Scout promotion or execution authority is implied.
