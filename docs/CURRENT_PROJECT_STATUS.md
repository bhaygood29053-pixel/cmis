...
The accepted live gate covered **3/3 repositories**, retrieved **41 provenance-shaped repository files**, completed **3/3 GitHub release queries**, and had **3/3 official X1 targets available**. It retained **10 architecture-analogue candidates**, all classified solely as `x1_program_schema`, and found **0 qualifying XONE allocation-source candidates / 0 exact-XONE-bound sources**.

The 10 surviving analogue artifacts are the expected X1 Labs airdrop-program architecture surfaces: `.env.example`, `Anchor.toml`, the ETH-only PDA migration design, program Cargo/source, PDA close script, on-chain PDA/type definitions, and generated IDL/type files. Every surviving row has `xone_named=false`, `exact_xone_contract_mentioned=false`, and `qualifying_xone_allocation_source_candidate=false`. These prove only that X1 has a concrete ETH-address-keyed airdrop/program architecture analogue; they do not bind XONE.

Before acceptance, manual inspection rejected and regression-tested several live false-positive classes: source-id terms leaking into evidence semantics, generic TypeScript `export`/config JSON, generic RPC/API URLs, the letters `pda` accidentally matching inside `update`, and IDL JSON being dual-classified as a holder allocation file. The final accepted source-class counts are: **10 x1_program_schema; 0 API; 0 Merkle; 0 content-addressed; 0 registry/export; 0 release asset; 0 structured allocation file**.

Evidence artifact id: `10033477532`; digest: `sha256:26e43c87579aad3abbd6932df2e54923f73e2f575abf8ab880a5070056d6811f`.

This proves only that the inspected bounded public repository/release/official-source classes did not expose the source of an XONE-derived XNT allocation. `private_or_unpublished_allocation_source_absence_proven=false`. `allocation_source_provenance_verified=false`, `authoritative_allocation_source_verified=false`, `official_xone_snapshot_verified=false`, `official_registry_artifact_verified=false`, `xone_snapshot_eligibility_verified=false`, `xone_snapshot_xnt_allocation_binding_verified=false`, `xnt_issuance_verified=false`, `xnt_vesting_or_unlock_verified=false`, `october_6_unlock_applies_to_xone_verified=false`, `xone_xnt_conversion_verified=false`, `cross_chain_correlation_verified=false`, and `execution_authorized=false`.

**Current next gate:** do **not** repeat the same broad public search. Move only on a newly surfaced or supplied exact primary artifact: XONE registry/export, holder-allocation CSV/JSON, authoritative API/Merkle pointer, or exact deployed X1 program/account schema explicitly tied to XONE. That source can then enter exact source verification and snapshot→XNT allocation-binding proof.

### Previous accepted live gate — X1-side XONE → XNT Allocation Record Discovery v1

**ACCEPTED INTERNAL FOUNDATION.** XONE XNT X1 Allocation Record Discovery run #7 passed deterministic and live jobs at PR #617 head `6d1b9bbedc7b21c0256fdff31c93407f250a053b`; Liquidity Scout Tests run #1823 also passed. PR #617 merged as `2499511af489173a4a37af76674d03be533d1b2d`; Issue #616 closed completed.

The accepted live source gate had **3/3 repositories available**, retrieved **15 ranked repository files**, and had **3/3 official X1 targets available**. The final accepted result was **0 allocation records, 0 candidates, 0 XONE-specific candidates, 0 exact-XONE-identity-bound candidates, 0 XNT-amount candidates, and 0 amount conflicts**.

The acceptance cycle deliberately rejected misleading evidence before merge. Earlier run #4 had produced 12 apparent pairs from `FairCrypto/x1-app/public/abi/XONE.json`. Inspection showed the alleged X1 pubkeys were Base58-looking slices of ABI/EVM bytecode, not typed X1 allocation keys. The parser was hardened to exclude ordinary ABI artifacts and bytecode-substring matches, and those cases became permanent deterministic regressions. The accepted run #7 then returned the clean zero above.

Evidence artifact id: `10031487515`; digest: `sha256:cd30ba9f18eb90c7ededecda6ab5b0949ed41aab567553575a008b8e20f663ac`.

This proves only that the inspected bounded public source set did not expose a qualifying Ethereum-address → X1-pubkey XONE allocation record. `private_or_unpublished_allocation_registry_absence_proven=false`. `official_xone_snapshot_verified=false`, `official_registry_artifact_verified=false`, `xone_snapshot_eligibility_verified=false`, `xone_snapshot_xnt_allocation_binding_verified=false`, `allocation_semantics_verified=false`, `claim_state_verified=false`, `vesting_or_unlock_state_verified=false`, `xnt_issuance_verified=false`, `xnt_vesting_or_unlock_verified=false`, `october_6_unlock_applies_to_xone_verified=false`, `xone_xnt_conversion_verified=false`, `cross_chain_correlation_verified=false`, and `execution_authorized=false`.

**Current next gate:** narrow to **allocation-source provenance** rather than repeat generic current-web or archive searches. Target exact XONE-linked registry/export/API/Merkle artifacts, released CSV/JSON holder-allocation files, or exact deployed X1 program/account schema references from authoritative/publicly supplied evidence. Any recovered source must still pass independent snapshot→allocation semantics before issuance or vesting claims.

### Previous accepted live gate — XONE Snapshot Archived Asset Graph Resolution v1
