...
The source registry remains intentionally narrow. The exact Ethereum mainnet XONE identity is accepted under `ethereum_xone_identity/v1`, bounded canonical ERC-20 event observation under `ethereum_xone_event_observer/v1`, burn/redeemer semantics under `ethereum_xone_migration_sink_semantics/v1`, exact-address Ethereum candidate extraction under `xone_xnt_conversion_candidate_discovery/v1`, independent X1-side XNT rule/account discovery under `x1_xnt_distribution_mechanism_discovery/v1`, exact X1-side binding discovery under `xone_xnt_x1_binding_discovery/v1`, and the Ethereum holder snapshot/registry foundation under `ethereum_xone_snapshot_registry/v1`. The snapshot/registry track is now the primary XONE→XNT investigation path. Run #1 searched 8 FairCrypto source/history targets, 4 official X1 targets, and 13 available X1 Report pages and found 0 explicit XONE snapshot claims / 0 official candidates / 0 authoritative exact snapshot blocks. That is scoped public-corpus evidence only; it does not prove that no private or unpublished snapshot exists. Earlier MoonParty work remains accepted historical background but is not the current roadmap priority and does not substitute for snapshot/allocation proof.
...
9. expand **snapshot provenance** through direct Jack/X1 historical posts or archives, XONE/X1 release artifacts, registry/CSV/JSON/IPFS references, Merkle roots, archived holder files and any X1 allocation list keyed by Ethereum XONE addresses;
10. when an authoritative exact snapshot block is found, run the accepted full Ethereum ledger reconstruction from XONE creation through that block, require historical `totalSupply()` and deterministic `balanceOf()` checks, and require two-RPC registry-digest agreement;
11. separately prove the **snapshot → XNT allocation binding**: ratio/formula, exclusions/minimums, Ethereum→X1 wallet mapping, claim/allocation record, and whether the allocation is already fixed;
12. separately prove XNT issuance/vesting/unlock semantics for **XONE-derived** allocations, including whether October 6 applies;
13. add deterministic Ethereum -> X1 correlation only after both exact sides are independently verified;
14. add persistent claim-ledger storage and supersession metadata;
15. only then consider any public CMIS or X1 Scout promotion.
