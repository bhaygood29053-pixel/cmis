...
The source registry remains intentionally narrow. The exact Ethereum mainnet XONE identity is accepted under `ethereum_xone_identity/v1`, bounded canonical ERC-20 event observation under `ethereum_xone_event_observer/v1`, burn/redeemer semantics under `ethereum_xone_migration_sink_semantics/v1`, exact-address Ethereum candidate extraction under `xone_xnt_conversion_candidate_discovery/v1`, independent X1-side XNT rule/account discovery under `x1_xnt_distribution_mechanism_discovery/v1`, exact X1-side binding discovery under `xone_xnt_x1_binding_discovery/v1`, and authoritative FairCrypto MoonParty source semantics under `xone_xnt_moonparty_source_semantics/v1`. The MoonParty source proof is materially stronger than the earlier bounded web corpus: it directly verifies a source-level design in which XONE is a redeemable/participating asset and the same contract exposes per-user and aggregate XNT-credit allocation accounting. Deployment, native-XNT equivalence, issuance and cross-chain settlement remain unverified. `moonparty_deployment_verification/v1` is also accepted internally via #601 / PR #602: its bounded live run covered 4/4 pinned sources, 6/6 current frontend targets and 26/28 same-host JS chunks across 35 documents, but found 0 exact MoonParty deployment-address candidates. That is scoped corpus evidence only, not proof of non-deployment.
...
7. **COMPLETE SOURCE-SEMANTIC BREAKTHROUGH:** FairCrypto MoonParty source semantics accepted under `xone_xnt_moonparty_source_semantics/v1` via #598 / PR #599. Pinned FairCrypto source directly verifies the design chain `XONE participation/redemption -> MoonParty burn/redeemer -> XNT credit allocation accounting`. This does not yet verify deployment or native-XNT equivalence;
8. **COMPLETE BOUNDED DEPLOYMENT DISCOVERY FOUNDATION:** `moonparty_deployment_verification/v1` accepted via #601 / PR #602. Run #2 inspected 35 pinned/current frontend documents and found 0 exact MoonParty deployment candidates; no global-absence inference is allowed;
9. expand MoonParty deployment provenance through additional authoritative FairCrypto/XEN release history, archived frontend/config material, package/deployment artifacts, verified explorer source, and provenance-qualified deployer history;
10. qualify any newly discovered exact deployment candidate with runtime compatibility, direct `XONE()` binding and two-RPC corroboration;
11. only after deployment identity is verified, read live `allocateXNTCredits` / `totalAllocatedXNTCredits` state and determine whether credits have an accepted mapping to native XNT issuance, vesting or claim state;
12. add deterministic Ethereum -> X1 correlation only after both exact sides are independently verified;
13. add persistent claim-ledger storage and supersession metadata;
14. only then consider any public CMIS or X1 Scout promotion.
