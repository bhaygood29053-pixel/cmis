...
8. identify the exact deployed MoonParty address and authoritative deployment chain, then verify runtime bytecode/ABI compatibility and direct `XONE()` binding to the accepted Ethereum XONE contract;
9. only after deployment identity is verified, read live `allocateXNTCredits` / `totalAllocatedXNTCredits` state and determine whether credits have an accepted mapping to native XNT issuance, vesting or claim state;
10. add deterministic Ethereum -> X1 correlation only after both exact sides are independently verified;
11. add persistent claim-ledger storage and supersession metadata;
12. only then consider any public CMIS or X1 Scout promotion.
