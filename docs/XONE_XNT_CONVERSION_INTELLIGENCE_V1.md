...
10. continue **primary-source snapshot recovery** outside the exhausted bounded current-public corpus: stable/direct Jack historical links or archives, archived X1/FairCrypto/XEN pages, registry/CSV/JSON/IPFS/Arweave artifacts, Merkle roots, archived holder exports and X1 allocation records keyed by Ethereum addresses;
11. when an authoritative exact snapshot block is found, run the accepted full Ethereum ledger reconstruction from XONE creation through that block, require historical `totalSupply()` and deterministic `balanceOf()` checks, and require two-RPC registry-digest agreement;
12. separately prove the **snapshot → XNT allocation binding**: ratio/formula, exclusions/minimums, Ethereum→X1 wallet mapping, claim/allocation record, and whether the allocation is already fixed;
13. separately prove XNT issuance/vesting/unlock semantics for **XONE-derived** allocations, including whether October 6 applies;
14. add deterministic Ethereum -> X1 correlation only after both exact sides are independently verified;
15. add persistent claim-ledger storage and supersession metadata;
16. only then consider any public CMIS or X1 Scout promotion.
