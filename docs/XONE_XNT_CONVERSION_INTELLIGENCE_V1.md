...
The source registry remains intentionally narrow. The accepted chain now includes exact Ethereum XONE identity/events/burn semantics, conversion-address discovery, X1 XNT mechanism/binding discovery, the Ethereum snapshot/registry verifier, current and archival snapshot provenance, archived asset-graph traversal, X1-side allocation-record discovery, and `xone_xnt_allocation_source_provenance/v1` via #619 / PR #620. Final allocation-source provenance run #6 covered 3/3 repositories, 41 provenance-shaped files, 3/3 release queries and 3/3 official X1 targets. It retained 10 clean `x1_program_schema` architecture analogues from `x1-labs/xenblocks-airdrop`, all explicitly non-XONE, and found 0 qualifying XONE allocation-source candidates / 0 exact-XONE-bound sources. No XONE-linked API, Merkle, content-addressed artifact, registry/export, release asset, structured holder-allocation file, or XONE-bound X1 program schema surfaced. This is scoped public-source evidence only and does not prove a private/unpublished allocation source is absent. The snapshot/registry track remains primary, but broad public discovery is now exhausted enough that the next work should be source-specific primary-artifact resolution/evidence intake, not another generic web/archive/repository pass. Earlier MoonParty work remains accepted historical background and does not substitute for snapshot/allocation proof.
...
14. wait for / resolve **source-specific primary allocation evidence** rather than repeating the exhausted broad corpus: exact XONE registry/export, holder CSV/JSON, authoritative API/Merkle pointer, content-addressed artifact, exact deployed X1 program/account schema explicitly tied to XONE, or externally supplied primary evidence;
15. when an authoritative exact snapshot block is found, run the accepted full Ethereum ledger reconstruction from XONE creation through that block, require historical `totalSupply()` and deterministic `balanceOf()` checks, and require two-RPC registry-digest agreement;
16. separately prove the **snapshot → XNT allocation binding**: ratio/formula, exclusions/minimums, Ethereum→X1 wallet mapping, claim/allocation record, and whether the allocation is already fixed;
17. separately prove XNT issuance/vesting/unlock semantics for **XONE-derived** allocations, including whether October 6 applies;
18. add deterministic Ethereum -> X1 correlation only after both exact sides are independently verified;
19. add persistent claim-ledger storage and supersession metadata;
20. only then consider any public CMIS or X1 Scout promotion.
