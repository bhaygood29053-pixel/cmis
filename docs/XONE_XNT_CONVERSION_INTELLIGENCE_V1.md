...
The source registry remains intentionally narrow. The exact Ethereum mainnet XONE identity is accepted under `ethereum_xone_identity/v1`, bounded canonical ERC-20 event observation under `ethereum_xone_event_observer/v1`, burn/redeemer semantics under `ethereum_xone_migration_sink_semantics/v1`, exact-address Ethereum candidate extraction under `xone_xnt_conversion_candidate_discovery/v1`, independent X1-side XNT rule/account discovery under `x1_xnt_distribution_mechanism_discovery/v1`, exact X1-side binding discovery under `xone_xnt_x1_binding_discovery/v1`, the Ethereum holder snapshot/registry foundation under `ethereum_xone_snapshot_registry/v1`, expanded current-public provenance under `xone_snapshot_provenance_expansion/v1`, bounded archive recovery under `xone_snapshot_archival_source_recovery/v1`, preserved application-asset traversal under `xone_snapshot_archived_asset_graph_resolution/v1`, and bounded X1-side allocation-record discovery under `xone_xnt_x1_allocation_record_discovery/v1`. Accepted allocation run #7 covered 3/3 repositories, 15 ranked repository files and 3/3 official X1 targets and produced 0 qualifying records / 0 candidates after ABI/bytecode false positives from an earlier run were rejected and regression-tested. This is scoped public-source evidence only; it does not prove a private/unpublished allocation registry does not exist. The snapshot/registry track remains primary. Earlier MoonParty work remains accepted historical background and does not substitute for snapshot/allocation proof.
...
13. expand **allocation-source provenance** without repeating generic web/archive scans: exact XONE-linked registry/export/API/Merkle artifacts, released CSV/JSON holder-allocation files, exact deployed X1 program IDs/account schemas tied to allocation logic, or newly supplied/public primary evidence;
14. when an authoritative exact snapshot block is found, run the accepted full Ethereum ledger reconstruction from XONE creation through that block, require historical `totalSupply()` and deterministic `balanceOf()` checks, and require two-RPC registry-digest agreement;
15. separately prove the **snapshot → XNT allocation binding**: ratio/formula, exclusions/minimums, Ethereum→X1 wallet mapping, claim/allocation record, and whether the allocation is already fixed;
16. separately prove XNT issuance/vesting/unlock semantics for **XONE-derived** allocations, including whether October 6 applies;
17. add deterministic Ethereum -> X1 correlation only after both exact sides are independently verified;
18. add persistent claim-ledger storage and supersession metadata;
19. only then consider any public CMIS or X1 Scout promotion.
